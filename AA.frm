...
Begin VB.Form formAddDocument 
   ...
   End
End
Attribute VB_Name = "formAddDocument"
...
FileCopy file_loc, CurrentDir & "\documents" & "\" & date_format & "\" & from_name & "\" & art_name & "_" & aut_name & "_" & art_date & "." & file_type
strr = ""
...
Open art_name & "_" & aut_name & "_" & art_date & "_" & file_type & ".afd" For Output As 1
    Write #1, from_name, date_format, art_name, aut_name, art_conf, art_date, art_keys, file_type, review_score, review_status, "documents"
Close
Dir1.Refresh
...
ResizeControls formAddDocument
End Sub
...
aa = retext(Text2.Text)
aa = retextQ(aa)
Text2.Text = retextEnter(aa)
End Sub

Private Sub Text4_Change()
aa = retext(Text4.Text)
Text4.Text = retextQ(aa)
End Sub
...
Public Function retextEnter(strr)
    If Len(strr) > 0 Then
        error_dot = InStr(1, strr, vbCrLf)
        If Len(strr) = error_dot Then
            strr = Mid$(strr, 1, Len(strr) - 1)
        ElseIf error_dot = 1 Then
            strr = Mid$(strr, 2, Len(strr))
